...
本阶段先完成前三个基础文档和一个可运行脚本，目标是让读者在不依赖大模型和高显存环境的情况下，先弄清楚视频生成到底比图像生成多了什么。
...
这会生成数据预览、采样 GIF、帧网格和指标文件。正式学习时建议从这个命令开始，而不是直接运行大规模视频模型。
...
| 顺序 | 文档 | 目标 |
| --- | --- | --- |
| 1 | [视频生成前置知识](01_video_generation_prerequisites.md) | 从图像扩散过渡到视频扩散，理解任务定义与核心难点 |
| 2 | [视频张量与数据集](02_video_tensors_and_datasets.md) | 学会把视频变成模型可用的张量，理解采样、帧率、窗口和预处理 |
| 3 | [Toy Video Diffusion](03_toy_video_diffusion_from_scratch.md) | 运行真实训练脚本，比较逐帧 baseline 与 temporal convolution |
...
这种设置生成效果不会惊艳，但它能清楚说明视频扩散的核心问题：时间维度必须被建模。第一阶段的 `toy_video_diffusion.py` 已经实现了数据生成、DDPM 训练、采样、GIF 保存和 framewise/temporal 两个 baseline。
