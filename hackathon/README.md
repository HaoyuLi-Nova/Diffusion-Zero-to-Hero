...
2. 打开并运行 [DreamBooth 笔记本](dreambooth.ipynb)（本仓库中文版）。在 Colab / Kaggle 等平台请选择 **GPU** 运行时。

| 笔记本 | 本地文件 |
|:-------|:---------|
| DreamBooth 训练 | [dreambooth.ipynb](dreambooth.ipynb) |

也可通过 [官方仓库](https://github.com/huggingface/diffusion-models-class/blob/main/hackathon/dreambooth.ipynb) 在 Colab / Kaggle / Gradient / Studio Lab 打开英文环境。
...
要参与评奖（已结束），需将 DreamBooth 模型推送到 Hub，并在模型卡片中加入 `dreambooth-hackathon` 标签（[示例](https://huggingface.co/lewtun/ccorgi-dog/blob/main/README.md#L9)）。[DreamBooth 笔记本](dreambooth.ipynb) 会自动创建该标签；若使用自有脚本需自行添加。
