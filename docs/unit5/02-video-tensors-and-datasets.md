...
如果你只想检查视频数据生成、张量形状和 GIF 保存，不需要训练很久，可以运行：
...
python unit5\toy_video_diffusion.py `
  ...
  --square-size 4 `
  --frames 4 `
  --output-dir unit5_outputs\data_smoke_test
```

这条命令会生成：

```text
unit5_outputs/data_smoke_test/dataset_preview.gif
unit5_outputs/data_smoke_test/dataset_preview_grid.png
```

如果这两个文件能正常打开，并且你看到一个彩色方块连续运动，说明数据生成、帧顺序、归一化和保存流程基本正确。

也可以只导入 dataset 做 shape 检查：

```python
from unit5.toy_video_diffusion import MovingSquareDataset
...
print(video.shape)        # torch.Size([8, 3, 32, 32])
print(video.dtype)        # torch.float32
print(video.min().item()) # -1.0
print(video.max().item()) # <= 1.0
```

这段代码是后续训练脚本真正使用的数据入口。
...
下面是移动方块数据集的核心逻辑，来自 `unit5/toy_video_diffusion.py`，也是训练脚本实际调用的数据实现：
